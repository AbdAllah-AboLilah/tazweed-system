...
// ⚠️⚠️ الدرجة الأساسية بتاخد اسم المجموعة **زي أي درجة** — اتبلّغ بالنص:
//   "لما اجي اطبع ملصق درجة من الدرجات الاساسيه ... مش بتطبع ب اسم
//    المجموعة زي الدرجات العاديه"
//
// وقبل كده كان فيه استثناء بيشيل المجموعة من الدرجات الأساسية، وكان
// **صح في وقته**: الدرجات الأساسية كانت بتاعة الفئة كلها فمكانش ليها
// مجموعة أصلًا. وبعدين بقت لكل مجموعة درجاتها الأساسية (v0.43.0) —
// والاستثناء فضل مكانه، فالملصق بقى يقول "أبيض" من غير ما يقول أبيض
// بتاع أنهي مجموعة، والفئة فيها أكتر من مجموعة.
//
// ⚠️ والدرجة الأساسية اللي **مالهاش** مجموعة لسه بتطلع باسمها لوحده —
// الشرط تحت بيمسك دي لوحدها.
...
// ============================================================
// ⚠️⚠️ "مركّز على الخانة" غير "بيكتب دلوقتي" — والفرق ده كان بيعطّل
//      زرار الريفريش عشر دقايق
// ============================================================
// اتبلّغ بالنص: "عاوزك تتاكد ان زرار الري فريش شغال كويس لان بحس ان
// مش بيشتغل كويس".
//
// السبب اللي اتقاس: isUserTyping فوق بترجّع true لمجرد إن الخانة
// **مركّز عليها** — حتى لو محدش لمسها من ساعة. ودي القاعدة الصح
// لإعادة الرسم (المؤشر بيضيع لو هدّينا الشاشة والخانة مركّز عليها).
//
// لكن مسار الريفريش (waitThenReload في print-core، وreloadWhenSafe في
// update-prompt) كان بيستخدم **نفس** الدالة. وشاشة الطباعة بتفتح
// وخانة البحث مركّز عليها وبتفضل كده. النتيجة:
//
//   تبعت ريفريش لجهاز واقف على شاشة الطباعة
//   → الجهاز يقول "فيه حد بيكتب" (وهو مفيش حد أصلًا)
//   → يستنى 600 ثانية كاملة قبل ما ينفّذ
//
// يعني الزرار **بيشتغل**، بس بعد عشر دقايق — واللي بيبصّ بيقول
// "مش شغّال". ونفس الفخ بالحرف كان بيأخّر التحديث التلقائي كمان.
//
// الحل: الانتظار يبقى على الكتابة **الفعلية** — آخر ضغطة زرار أو آخر
// تغيير في الخانة. خانة مركّز عليها ومحدش بيكتب فيها = مفيش حاجة تتحمى.
//
// ⚠️ وisUserTyping نفسها **ماتغيّرتش**: renderFromData لسه شغّالة
// بيها بالظبط، لأن ضياع المؤشر بيحصل بالتركيز لوحده.
const TYPING_IDLE_MS = 20000;
let lastTypeAt = 0;

// ⚠️ 'input' مع 'keydown': كيبورد أندرويد بيبعت keydown بكود 229 مش
// مفيد، لكن 'input' بيحصل مع كل حرف على كل الأجهزة.
// و`true` (مرحلة الالتقاط) عشان أي حقل يوقف الحدث مايخفيهوش عننا.
['keydown', 'input'].forEach((ev) => {
  document.addEventListener(ev, () => { lastTypeAt = Date.now(); }, true);
});

function isUserTypingNow() {
  if (!isUserTyping()) return false;
  return Date.now() - lastTypeAt < TYPING_IDLE_MS;
}
...
function sideMenuHTML() {
  // ============================================================
  // ⚠️⚠️ الدرج المقفول مابيتبنيش — بيتبني لما يتفتح
  // ============================================================
  // على الموبايل القايمة دي **درج**: مقفول يبقى برّه الشاشة ومحدش شايفه.
  // ومع كده كانت بتتبني كاملة (39 فئة × 3 زراير) مع **كل** رسمة —
  // يعني مع كل ضغطة زاد/ناقص وانت في الشيت.
  //
  // القياس على معالج مبطّأ 4 أضعاف (39 فئة · 80 درجة):
  //
  //   بالدرج ....... 2093 عنصر → 309ms
  //   من غيره ...... 1844 عنصر → 267ms
  //   ─────────────────────────────────
  //   الوفر ........ 249 عنصر → **42ms (14%)**
  //
  // ⭐ وطول الصفحة **مايتغيّرش** (18574 في الحالتين) — الدرج بيقعد فوق
  // الشاشة مش جواها، فشيله مالوش أي أثر على التمرير ولا على أي حاجة
  // شايفها.
  //
  // ⚠️ وكل زرار بيفتح الدرج بيعمل render() بعده على طول، فالمحتوى
  // بيتبني قبل ما يبان بجزء من الثانية. اتأكدنا من ده في 8 أماكن.
  //
  // ⚠️⚠️ وعلى الكمبيوتر الدرج **مش درج** — ده عمود ثابت ظاهر دايمًا،
  // فالشرط `state.isNarrow` أساسي. من غيره القايمة تختفي من على شاشة
  // الكمبيوتر خالص.
  if (state.isNarrow && !state.sideMenuOpen) {
    return '<aside class="side-menu" id="side-menu"></aside>';
  }
...
// ⚠️ كان 100 ("بحد اقصي 100")، واتطلب يبقى 400 بعد التجربة:
// "زود الحد الاقصي بتاع السجل الي ٤٠٠ عرض".
//
// 400 لسه تحت السقف المطلق (LOG_FETCH_MAX = 2000)، وهو نفس الرقم
// اللي النظام كان بيجيبه **في كل فتحة** قبل إصلاح v0.95.0 —
// بس دلوقتي باختيارك مش غصب عنك، والافتراضي لسه 50.
const LOG_PAGE_MAX = 400;
...
async function registerPrintStation() {
  ...
  // ============================================================
  // ⚠⚠ الجهاز موجود لو فيه **أي** طريق للطباعة
  // ============================================================
  // العطل اللي بيتحل، اتبلّغ بالنص:
  //   "النظام بيبقى مفتوح عادي وبلاقي في الاعدادات من الهاتف مقفول"
  //   "الجهاز المقفول له كارت واحد ... مش عارف ايه السبب اني
  //    قافل برنامج QZ ولا حاجه تانيه"
  //
  // وتخمينه كان صح: السطر كان `if (!(await ensureQZConnected())) return;`
  // يعني النبضة كلها بتقف لو QZ مش شغّال، فـlastSeen مابيتكتبش،
  // فبعد دقيقتين الجهاز بيبان "مقفول" للأبد.
  //
  // ⚠️ وده بقى تناقض من ساعة ما عملنا البرنامج المساعد: هو
  // اتعمل عشان **يستبدل** QZ، ومع ذلك الجهاز اللي شغّال
  // بالمساعد لوحده كان مايسجّلش نفسه خالص — يعني اللي شال QZ
  // التزم يرجّعه عشان جهازه يبان.
  //
  // السؤال الصح: "هل الجهاز ده يقدر يطبع؟" — والإجابة
  // أي واحد من الاتنين.
  const helperUp = typeof helperStatus === 'function' ? !!(await helperStatus()) : false;
  const qzUp = await ensureQZConnected();
  if (!helperUp && !qzUp) return;
    ...
    fireWrite(db.collection('printStations').doc(deviceId).set(
      {
        ...
        // ⚠️ نسخة QZ Tray على الجهاز ده — بتتنشر عشان تتشاف من التليفون.
        // اتطلبت وإحنا بنشخّص عطل قص ورقة التزويد: كل تشخيص اتبنى على
        ...
        printSetup: {
          ...
          // ⚠️⚠️ وخط الورقة جنبه — من غيره نافذة "إرسال إعدادات
          // الطابعة" مش هتعرف الجهاز شغّال بأنهي خط ورقة، فهتكتب
          // "زي ما هي (خط الجهاز)" حتى لو هو على المراعي.
          //
          // ⚠️ ودي القاعدة لأي إعداد جديد: يتحط في PRINT_FIELDS،
          // ويتنشر هنا، وتتعمل له خانة في نافذة الإرسال. اللي بيقع
          // منهم واحدة بيبقى إعداد **مش موجود من التليفون**.
          sheetFont: typeof getSheetFontId === 'function' ? getSheetFontId() : '',
          // ============================================================
  ...
  unsubActivityLog = db
    ...
    .onSnapshot(
      ...
      (snap) => {
        ...
        // ============================================================
        // كل تعديل كمية بيكتب سطر في السجل. والاشتراك ده بيسمع الكتابة
        // دي **مرتين** (واحدة محلية وواحدة لما السيرفر يأكّد، عشان
        // includeMetadataChanges) — وكان بيرسم **الشاشة كلها** في
        // المرتين.
        //
        // يعني وانت في الشيت بتدوس زاد/ناقص، السجل — اللي مش ظاهر أصلًا
        // على الشاشة دي — كان بيسبّب **رسمتين كاملتين زيادة لكل ضغطة**.
        //
        // ⚠️ والسجل بيتعرض في شاشتين بس: "العمليات" و"الرئيسية" (آخر 8
        // حركات). في أي شاشة تانية الرسم ده شغل ضايع بالكامل.
        //
        // ⚠️ ومابنوقفش تحديث `state.activityLog` نفسه — ده بيفضل يتحدّث
        // دايمًا، فأول ما تفتح السجل تلاقيه جاهز من غير انتظار.
        if (state.screen === 'activity' || state.screen === 'home') renderFromData();
      },
...
function init() {
  ...
  auth.onAuthStateChanged((user) => {
    ...
    unsubProfile = db.collection('users').doc(user.uid).onSnapshot((snap) => {
      ...
      // ⚠️⚠️ الإشعار البعيد بيعتمد على علامة الجهاز بيكتبها هو
      // (`wantsRestock`). فلو الصلاحية اتقفلت وهو فاتح، لازم يحدّثها
      // **دلوقتي** — من غير كده تليفونه يفضل يرن لحد ما يقفل النظام
      // ويفتحه، والسحابة بتثق في العلامة اللي هو كاتبها.
      //
      // ⚠️ والتسجيل كمان بيتعاد كل فتحة: توكن الإشعارات بيتغيّر لوحده
      // من المتصفح من وقت للتاني، والقديم بيبقى ميت في صمت.
      try {
        if (typeof syncPushAudience === 'function') syncPushAudience();
        if (typeof registerPushDevice === 'function') registerPushDevice();
      } catch (err) {
        console.warn('تعذّر تحديث تسجيل الإشعارات:', err);
      }
