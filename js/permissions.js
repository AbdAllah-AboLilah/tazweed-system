...
const PERMISSION_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        key: 'printerSetup',
        label: 'إعدادات الطابعة والمعايرة',
        hint: 'اختيار الطابعة، الإعدادات المتقدمة، ومعايرة الملصق',
        danger: true,
      },
      // ⚠️ ليه ده مفتاح لوحده مش مع اللي فوقه؟
      //
      // "إعدادات الطابعة" معناها **يظبط الماكينة اللي قدامه** — وده
      // معقول تفتحه لواحد واقف عند الكاشير.
      //
      // ده معناه **يتحكم في كل أجهزة المحل من تليفونه**، ويغيّر إعداد
      // على ماكينة هو مش شايفها ولا واقف عندها. سلطة تانية خالص، فمفتاح
      // تاني — ومقفول لكل الرتب غير منشئ النظام.
      {
        key: 'remoteControl',
        label: 'التحكم في أجهزة الطباعة عن بُعد',
...
const ROLE_PRESETS = {
  ...
  [ROLES.BRANCH_MANAGER]: {
    ...
    manageUsers: false,
    // إعدادات الطابعة كمان: دي بتلمس عتاد حقيقي (معايرة الطابعة)، فخلّيناها
    // على صاحب المحل بس. تتفتح لشخص بعينه لو احتجت.
    printerSetup: false,
