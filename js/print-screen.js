  ...
  return `
    <div class="dev-card ${online ? '' : 'off'}">
      ...
      <div class="dev-lines">
        ...
        ${
          // ⚠️ مقاس الورق في تعريف الويندوز. نفس سبب النسخة فوق: التفسير
          // الفاضل لقص ورقة التزويد هو إن الفورم مختلف من جهاز لجهاز،
          // والرقم ده هو اللي هيأكّده أو يقتله بدل تخمين خامس.
          st.restockPaper
            ? `<div><span>ورق التعريف</span><b>${escapeHTML(st.restockPaper)}</b></div>`
            : ''
        }
      </div>
...
// ⬇️ رابط تحميل، وخلاص
// ============================================================
// ⚠️⚠️ ده الشكل **التالت** للزرار ده، واتبلّغ بالنص: "زرار تحميل
// المساعد مش شغال بردوا وانا زهقت حط مكانه لينك تحميل المساعد لما
// اضغط عليه يفتح في المتصفح ويحمل وخلاص".
//
// اللي اتجرّب وفشل:
//   ١) <a download> على رابط الموقع   → مافيش أي رد فعل
//   ٢) تحميل بالجافاسكريبت بشريط تقدم → برضه مافيش
//
// وكل واحد فيهم كان بيضيف طبقة بينه وبين المتصفح. فبطّلنا طبقات:
// رابط عادي بيفتح تبويب جديد على الملف نفسه، والمتصفح بيتصرف.
//
// ⚠️ والرابط على **raw.githubusercontent** مش على موقعنا عن قصد:
//   • بيعدّي من غير أي علاقة بالـService Worker بتاعنا
//   • وبيرد بنوع `application/octet-stream` فالمتصفح بينزّله على طول
//   • وهو نفس الرابط اللي البرنامج بيحدّث نفسه منه — يعني مجرّب فعلًا
const HELPER_EXE_URL =
  'https://raw.githubusercontent.com/AbdAllah-AboLilah/tazweed-system/main/helper/dist/tazweed-helper.exe';
...
function openHelperDownloadDialog() {
  ...
  overlay.innerHTML = `
    <div class="card" style="max-width:400px; width:100%; max-height:92vh; overflow:auto;">
      ...
      <a class="btn btn-primary" id="helper-dl-link" href="${HELPER_EXE_URL}"
  ...
  // ============================================================
  // ⚠️⚠️ الضغطة لازم يبقى ليها **رد فعل**، حتى لو المتصفح رفض
  // ============================================================
  // اتبلّغ بالنص: "لما اضغط علي زرار برنامج المساعد من التليفون
  // ميفتحش حاجه ولا يديني ردت فعل".
  //
  // والسبب مش عندنا: متصفح الموبايل بيرفض ملفات البرامج (.exe) —
  // بيبلع الضغطة من غير ما يقول حاجة. مافيش في الجافاسكريبت أي طريقة
  // نعرف بيها إن التحميل اتمنع، لأن الرابط بيسيب صفحتنا خالص.
  //
  // فبنعمل اللي نقدر عليه: **نقول اللي المفروض يحصل**. كده اللي
  // مايشوفش حاجة يعرف إن ده رفض من المتصفح مش عطل في النظام،
  // ويعرف يعمل إيه.
  //
  // ⚠️ ومابنمنعش الضغطة على الموبايل: يمكن متصفحه يقبلها، وقرار
  // المنع مش بتاعنا.
  const dlLink = overlay.querySelector('#helper-dl-link');
  if (dlLink) {
    dlLink.addEventListener('click', () => {
      const onPhone = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent || '');
      if (onPhone) {
        showPrintNotice(
          '📥 التحميل بدأ في تبويب جديد. لو مافتحش أي حاجة، يبقى متصفح ' +
            'التليفون رفض ملف البرنامج — وده طبيعي. نزّله من الكمبيوتر اللي ' +
            'عليه الطابعة، هو مالوش لازمة على التليفون أصلًا.',
          16000,
          'warn'
        );
      } else {
        showPrintNotice('📥 التحميل بدأ في تبويب جديد — دوّر عليه في مجلد التنزيلات.', 9000);
      }
    });
  }

  // ⚠️ رقم النسخة **زيادة مش أساس**: لو النداء فشل، النافذة زي ما هي
  // والرابط شغّال. عشان كده مافيش أي رسالة خطأ هنا.
