...
//
//   ٣) **تقديم ورق في الآخر**. من غير التعريف مافيش حد بيقدّم الورق،
//      فآخر سطر بيفضل جوّه الماكينة ومش هتعرف تقطعه. مافيش أمر قص —
//      لو الطابعة مالهاش قصّاصة الأمر ممكن يطبع رموز غريبة.
//
// بترجّع true لو اتبعتت، false لو محتاجين نرجع للطريقة العادية.
const SHEET_RAW_FEED = '\x1B\x64\x06'; // ESC d 6 = قدّم 6 سطور
...
async function printSheetRaw(printerName, image) {
  ...
  try {
    ...
    await qz.print(config, [
      ...
      { type: 'raw', format: 'command', flavor: 'plain', data: SHEET_RAW_FEED },
    ]);
    return true;
  } catch (err) {
