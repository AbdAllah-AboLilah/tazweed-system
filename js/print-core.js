...
// ============================================================
// ⭐⭐ ورقة التزويد: مقاس الصفحة
// ============================================================
// ورقة التزويد بتتقاس قبل الطباعة عشان نعرف تدخل في ورقة تعريف
// الطابعة (297مم) ولا لأ. القياس الحقيقي (عرض 80مم):
//   30 درجة  →  62مم   ✅ تدخل، بنبعت مقاسها
//   80 درجة  → 125مم   ✅
//  165 درجة  → 236مم   ✅ بالعافية
//  245 درجة  → 324مم   ❌ أطول من الفورم → مانبعتش مقاس، الدرايفر يصغّر
//
// ⚠️ ماتفتكرش إن بعت الطول الحقيقي بيدّيك ورقة أطول — **مابيحصلش**.
// الشرح الكامل والقياس عند RESTOCK_FORM_HEIGHT_MM تحت.
//
...
// اللي الطابعة عليه.
// ⚠️⚠️ الـ80 دي **عرض الورقة**، مش عرض اللي بيتطبع. رأس الطباعة بيغطي
// 72.1مم في النص بس (اسم التعريف نفسه "80(72.1)")، يعني ~4مم من كل
// ناحية مابيتطبعوش. عشان كده جسم ورقة التزويد لازم يفضل **في النص**
// (margin: 0 auto في print-restock.js) — من غيرها بيتلزق في حرف اليمين
// وأرقام أول عمود بتتاكل. ماتغيّرش الرقم ده من غير ما تراجع الاتنين مع
// بعض.
const RESTOCK_PAGE_WIDTH_MM = 80;
// ============================================================
// ⭐⭐⭐ الطابعة **مابتاخدش** الطول اللي بنبعتهولها. مقيس على ورق.
// ============================================================
// ده أهم سطر في الملف كله. اقراه قبل ما تلمس المقاس.
//
// في v0.55.0 بعتنا للطابعة الطول الحقيقي للورقة عشان الورقة الطويلة
// ماتصغّرش. النتيجة على الورق كانت **أسوأ**: الورقة اتقصّت من تحت.
//
// القياس اللي حسم الموضوع (ورقة مودال سفنجة، 245 درجة):
//   طول الورقة الكامل        = 324.1 مم
//   اللي بعتناه للطابعة      = 359   مم
//   اللي اتطبع فعلًا         = 294.4 مم  ← آخر صف كامل قبل 297
//   المقاس في تعريف الطابعة  = 297   مم
//
// ودليل تاني ماينفعش نتجاهله: طبعنا نفس الورقة مرتين بطولين مختلفين
// (+2مم و+8%+8مم) — و**اتقصّت في نفس الصف بالظبط** (صف 210). يعني
// الرقم اللي بنبعته مالوش أي أثر على طول الصفحة.
//
// الخلاصة: الدرايفر ماشي على الفورم بتاعه (297مم) مهما بعتنا. إحنا لما
// بعتنا مقاس، اللي عملناه فعلًا إننا **وقّفنا التصغير التلقائي** —
// فبدل ما الورقة تصغّر وتخلص كاملة، بقت تتطبع بحجمها وتتقص عند 297.
//
// عشان كده القاعدة بقت:
//   • الورقة اللي **بتدخل** في الفورم → نبعت مقاسها (دقيق، مفيش تصغير)
//   • الورقة اللي **أطول** من الفورم → **مانبعتش مقاس خالص**، ونسيب
//     الدرايفر يصغّرها زي ما كان بيعمل قبل v0.55.0. صغيرة وكاملة أحسن
//     من كبيرة ومقصوصة.
//
// ⚠️ ومابنقسّمش الورقة: المستخدم طلب ورقة واحدة صراحةً ("لو انا طالب
// الورقة تطبع كلها مرة واحده يرجع لتصغير ورقة التزويد لتدخل في حجم
// الورقة مش يقسمها"). اللي عايز الحجم الكامل عنده خيار "كل مجموعة في
// ورقة لوحدها".
//
// 📌 لو صاحب المحل غيّر المقاس الافتراضي في تعريف الطابعة في ويندوز
// (القايمة فيها مقاسات لحد 80 × 3276مم)، الرقم ده هو اللي يتظبط هنا.
const RESTOCK_FORM_HEIGHT_MM = 297;
...
// عدّى أطول مقاس متاح.
// ============================================================
// ⭐ هامش الطول — صغير، وده مقصود
// ============================================================
// كنا خايفين إن محرك QZ يرسم أطول من قياس المتصفح فحطينا 8%. القياس
// على الورق طلّع العكس: آخر صف اتطبع كان بينتهي عند 294.4مم في قياسنا،
// والطابعة طبعته كامل ووقفت — يعني **قياسنا مطابق للورق في حدود صف
// واحد**. فالهامش الكبير كان بيضيّع ورق على الفاضي.
const RESTOCK_HEIGHT_PAD_MM = 4;

// بترجّع مقاس الصفحة لورقة التزويد، أو null يعني **مانبعتش مقاس** —
// وساعتها الدرايفر بيصغّر الورقة عشان تدخل، وهو المطلوب للورقة الطويلة.
async function restockPageSize(html) {
  const mm = await measureHTMLHeightMm(html, RESTOCK_PAGE_WIDTH_MM);
  if (!(mm > 0)) return null;
  const height = mm + RESTOCK_HEIGHT_PAD_MM;
  // أطول من ورقة التعريف؟ سيب الدرايفر يصغّرها. لو بعتنا مقاس هنا،
  // التصغير بيتوقف والورقة بتتقص عند 297 — وده اللي حصل فعلًا.
  if (height > RESTOCK_FORM_HEIGHT_MM) return null;
  return { width: RESTOCK_PAGE_WIDTH_MM, height };
}
