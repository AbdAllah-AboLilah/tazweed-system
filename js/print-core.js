...
// عدّى أطول مقاس متاح.
// ============================================================
// ⭐ هامش الطول — بنزوّد عن قصد، ومش هزار
// ============================================================
// القياس بيحصل بمحرك **المتصفح** (كروم)، لكن اللي بيرسم الورقة على
// الورق فعلًا هو **محرك QZ** (WebView بتاع Java). محركين مختلفين =
// أطوال سطور مختلفة شوية، والفرق بيتراكم: ورقة فيها 400 صف، فرق عُشر
// مللي في الصف = 4 سنتي في آخر الورقة.
//
// والتكلفة مش متساوية:
//   • زوّدنا أكتر من اللازم → شوية ورق أبيض في الآخر. مالوش أي أثر.
//   • قصّرنا شعرة        → **آخر الدرجات بتتقص وماتطبعش خالص**.
//
// عشان كده بنزوّد 8% + 8مم. ورقة 250مم بتبقى 278، وورقة 600 بتبقى 656.
// (كان الهامش 2مم بس، والورقة رجعت من الطابعة ناقصة درجات — v0.55.1)
const RESTOCK_HEIGHT_SAFETY = 1.08;
const RESTOCK_HEIGHT_PAD_MM = 8;

async function restockPageSize(html) {
  const mm = await measureHTMLHeightMm(html, RESTOCK_PAGE_WIDTH_MM);
  if (!(mm > 0)) return null;
  const safe = Math.ceil(mm * RESTOCK_HEIGHT_SAFETY) + RESTOCK_HEIGHT_PAD_MM;
  if (safe > RESTOCK_MAX_HEIGHT_MM) return null;
  return { width: RESTOCK_PAGE_WIDTH_MM, height: safe };
}
