...
// ============================================================
// ⚠️⚠️ الفشل بيتخزّن **مؤقتًا** — النجاح بس اللي بيفضل للجلسة
// ============================================================
// العطل اللي بيتحل، اتبلّغ بالنص: "بردوا في ورق لما يتبعت بيجي امر
// الاول علي جهاز الكمبيوتر ويظهر المعاينه" — وصاحب النظام أكّد إن
// "النظام علي الكمبيوتر علي احدث نسخة ومتصل بالمساعد".
//
// والاتنين صح مع بعض: البرنامج شغّال **دلوقتي**، بس النظام سأل عنه
// مرة واحدة زمان ولقاه مش شغّال، وخزّن الإجابة دي **للأبد**.
//
// السطر اللي كان بيعمل كده:  if (helperCache !== null) return helperCache;
// فأول فحص فاشل بيقفل الباب على البرنامج لحد ما الصفحة تترفرش.
//
// وده بيحصل كل يوم في المحل: الكمبيوتر بيقوم، المتصفح بيفتح مع
// الويندوز، والبرنامج المساعد لسه بيشتغل. أول طبعة بتلاقيه مش جاهز،
// وباقي اليوم كله بيروح لنافذة المتصفح — والورقة بتتقص.
// وكفاية كمان إن الرد ياخد أكتر من 1.2 ثانية مرة واحدة (جهاز مشغول)،
// أو إن حد يقفل البرنامج ويفتحه تاني.
//
// ⚠️ التكلفة على السرعة **صفر في الحالة الطبيعية**: البرنامج شغّال →
// النجاح بيتخزّن للجلسة زي ما كان بالظبط، فحص واحد وخلاص. اللي بيدفع
// هو الجهاز اللي **مافيهوش** البرنامج: فحص واحد كل 20 ثانية بدل فحص
...
async function deliverPrint(type, html, sizeOptions, winFeatures, browserHTML, spec) {
  ...
  const noPrinter = why.includes('مش مظبوط على طابعة');

  // ============================================================
  // ⚠️⚠️ ورقة التزويد في نافذة المتصفح = **ورقة مقصوصة**
  // ============================================================
  // ده مش إزعاج، ده فقدان بيانات. نافذة المتصفح بتطبع بمقاس الصفحة
  // بتاع التعريف، وورقة التزويد رول مستمر طوله مفتوح — فبتتقسّم
  // وبتتقص عند حد الفورم.
  //
  // ⚠️ العطل ده اتبلّغ مرتين وضيّعنا فيه أيام ندوّر في الرسم والقياس:
  // الورقة كانت بتقف عند **211.7مم** بالظبط في المرتين (قِسناها: الصف
  // اللي بينتهي هناك هو آخر صف طالع). والسبب طلع إن الطباعة المباشرة
  // فشلت على الجهاز ده أصلًا (الطابعة مش متظبطة/QZ مش شغّال)، فالورقة
  // راحت لنافذة ويندوز — واحنا كنا بندوّر في الصورة وهي مالهاش دعوة.
  //
  // فالتنبيه للورقة **بيقول القص صراحةً وبيقعد أطول**: اللي يشوف
  // "هتتفتح نافذة عادية" بيدوس طباعة ويمشي، واللي يشوف "الورقة هتتقص"
  // بيروح يظبّط الطابعة.
  const willBeCut = type === 'restock';
  showPrintNotice(
    willBeCut
      ? '⚠️⚠️ ورقة التزويد **هتطلع مقصوصة**! الطباعة المباشرة ماتمّتش على الجهاز ده' +
          `${noPrinter ? ' (طابعة ورقة التزويد مش متظبّطة)' : why ? ` (${why})` : ''}` +
          '، ونافذة ويندوز بتقص الورقة عند مقاس الصفحة. ظبّط الطابعة من ⚙️ إعدادات الطابعة وتأكد إن QZ Tray شغّال.'
      : noPrinter
...
// من غير سبب.
// ============================================================
// 🗂️ المفاتيح متقسّمة مجموعات
// ============================================================
// اتطلب بالنص: "في لغبطه في الاعدات المتقدم في مفاتيح كتير وحاسس ان
// في زاراير بتعمل نفس المهام عاوزك تقسمها".
//
// كانوا قايمة واحدة طولها ١٥ مفتاح ورا بعض من غير أي فاصل، ومفيش حاجة
// بتقول أنهي مفتاح ليه علاقة بأنهي. والمفاتيح فعلًا **مش نوع واحد**:
// فيه اللي بيقرّر الطبعة رايحة لمين، وفيه اللي بيرسم الملصق، وفيه
// خيارات بتتبعت لـQZ ومالهاش أي أثر لو الطباعة مش ماشية عليه أصلًا.
//
// ⚠️ التقسيم ده **شكل بس** — ولا مفتاح اتغيّر سلوكه ولا اسمه ولا
// قيمته المحفوظة.
//
// ⚠️⚠️ ومجموعة 'hidden' معناها إن المفتاح **بيتعرض في مكان تاني** أو
// اتدمج في غيره، مش إنه اتشال: قيمته لسه بتتقرا وبتتنسخ بين الأجهزة.
const PRINT_TWEAK_GROUPS = [
  {
    key: 'route',
  ...
  // مابقاش يزاحم في الشاشة. (نفس اللي عملناه مع sheetImage في v0.84.0.)
  {
    ...
    hint:
      'مين بيرسم الملصق — محرّك QZ ولا إحنا. ⚠️ البرنامج المساعد ' +
      'بيطبع **صور** بس، فالملصق اللي بيتبعت كنص بيروح لـQZ مهما كان ' +
      'مفتاح البرنامج مفتوح.',
  },
  {
    key: 'shape',
    title: '📏 مقاسات الملصق وشكله',
    hint: 'حجم الحروف والباركود جوه اللاصقة. مالهاش دعوة بطريق الطباعة.',
  },
  {
    key: 'qz',
    title: '⚙️ خيارات QZ Tray',
    hint:
      'بتتبعت لـQZ مع أمر الطباعة. ⚠️ مالهاش **أي أثر** على اللي بيخرج ' +
      'من البرنامج المساعد — هو مابيعديش على QZ أصلًا.',
  },
  { key: 'info', title: '🧭 معلومات على الشاشة', hint: 'مابتغيّرش الطباعة نفسها.' },
];
...
const PRINT_TWEAKS = [
  ...
  },
  {
    ...
    key: 'tightLabelPad',
    group: 'shape',
    label: '📐 قلّل الفراغ فوق وتحت (تجريبي)',
    hint:
      'قيسنا ملصق حقيقي: **نصّه فراغ** (حبر 6.4مم مقابل فراغ 6.1مم). ' +
      'المفتاح ده بيرجّع جزء من الفراغ اللي فوق واللي تحت للاسم، ' +
      'فالاسم الطويل يقدر يوصل للمقاس اللي في الخانة. ' +
      '⚠️ الـQR والباركود والسعر **والهامش الجانبي** ماتلمسوش خالص.',
    defaultOn: false,
    apply: () => {},
  },
  {
  ...
  },
  {
    ...
    // ============================================================
    // 🧪 ورقة التزويد **خام** — من غير ما تعدّي على تعريف الويندوز
    // ============================================================
    // المشكلة اللي بيحلها ده، بالأرقام المقيسة (v0.55.3، على ورق حقيقي):
    //
    ...
    //
    // يعني الطابعة مابتاخدش الطول اللي بنبعتهولها. اللي بيحصل إن الطول
    // الصريح **بيقفل التصغير التلقائي** بتاع التعريف — فبدل ما الورقة
    // تصغّر وتخرج كاملة، بتتقص عند فورم التعريف. وفورم التعريف بيختلف من
    // جهاز لجهاز، وده اللي بيفسّر إن نفس النظام ونفس نسخة QZ ونفس موديل
    // الطابعة يطلّعوا نتيجتين مختلفتين (211.7مم على جهاز، وكاملة على تاني).
    //
    // ⚠️ و`size.custom` (QZ 2.2.6) المفروض يتخطّى ده — بس التلات أجهزة
    // كلهم على 2.2.6 والقص لسه بيحصل. يعني تعريف Xprinter مش بيحترمه.
    //
    // **الحل هنا**: مانعدّيش على موديل الصفحة بتاع الويندوز أصلًا. بنبعت
    // الصورة **خام** (ESC/POS) زي ما إيصال الكاشير بيتبعت بالظبط —
    // الطابعة بتطبع نقط لحد ما البيانات تخلص. مافيش فورم ورق يقص، ومافيش
    // تصغير، وإعدادات الويندوز بتبقى خارج الموضوع تمامًا.
    //
    // ⚠️⚠️ الرسالة بتفضل صغيرة: بنبعت **الصورة PNG** وQZ هو اللي بيحوّلها
    // لـESC/POS جوّه. لو حوّلناها إحنا كنا هنبعت ~347 كيلو من النقط
    // الخام — أكبر من حد الرسالة (48 كيلو) بسبع مرات، وكانت هتتضاع في صمت.
    //
    // مقفول افتراضيًا: محتاج تجربة على ورق حقيقي.
    // ============================================================
    // 📏 اسم الصنف بمقاس ثابت — الشرح الكامل في print-label.js
    // ============================================================
    // القياس: الاسم بيتغيّر من 2.70مم (10 حروف) لـ1.29مم (75 حرف) —
    ...
    hint:
      ...
      'على الرقم اللي في الخانة اللي تحته (الافتراضي 1.9مم = أكبر رقم ' +
      'يخلي كل الأسماء متساوية)، والاسم بينزل لسطر تاني وتالت لو ' +
  ...
  },
  {
    ...
    // 🪶 الـQR بـ1 بت
    // ============================================================
    // القياس على ملصق حقيقي: الملصق 7,222 بايت، **منهم 3,556 صورة
    // الـQR** (49%). وحد رسالة QZ 48 كيلو، فده بيحدّد كام ملصق يدخلوا
    // في الأمر الواحد.
    //
    // ⚠️ النقط مابتتغيّرش — نفس الصورة متخزّنة صح بس (الشرح عند
    // canvasToPng1Bit). ومفتاح لوحده لأن الـQR هو اللي بيتقرا.
    key: 'lightQR',
    group: 'shape',
    label: '🧪 صورة الـQR أخف (تجريبي)',
    hint:
      'صورة الـQR بتتحفظ دلوقتي بـ32 لون وهي أصلًا لونين. المفتاح ده ' +
  ...
  },
  {
    ...
    hint:
      ...
      'إيصال الكاشير بالظبط، من غير ما تعدّي على مقاس الورق المتظبط في ' +
      'تعريف الويندوز — فمابتتقصش لما تطول. ' +
      '⚠️ لو البرنامج المساعد شغّال ومفتاحه مفتوح، المفتاح ده مالوش لازمة. ' +
      'ولو فشل بيرجع للطريقة العادية لوحده وبيقولك.',
    apply: () => {},
  },
  {
    key: 'sheetImage',
  ...
  {
    // ============================================================
    // 📏 مقاسات ثابتة للسعر ورقم الباركود
    // ============================================================
    // العطل اللي اتبلّغ: "كل ملصق غير التاني في حجم الخط وتنسيقه".
    //
    // والقياس أكّده: السعر والرقم بيفرقوا **20%** بين ملصق وملصق —
    // ومش بانتظام كمان. اسم أطول طلّع رقم **أكبر** (2.32 مقابل 2.02)،
    // فمفيش قاعدة تقدر تتوقعها.
    //
    // السبب: التلاتة (الاسم + الرقم + السعر) بيتحسبوا في معادلة واحدة،
    // فلما الاسم ياخد سطرين الكل بيصغّر بنفس النسبة — يعني الاسم بيجرّ
    // السعر والرقم معاه.
    //
    // المفتاح ده بيفك الربط ده: الرقم والسعر بمقاس ثابت، والاسم بياخد
    // اللي فاضل. وبيضيّق المسافة بين الرقم والسعر عشان يرجّع للاسم
    // اللي أخده منه التثبيت.
    //
    // ⚠️ مطفي افتراضيًا لحد ما يتجرّب على ورق. وقفله بيرجّع الملصق
    // **بايت ببايت** زي ما كان — فيه فحص بيقارن النصين حرف بحرف.
    key: 'fixedLabelSizes',
    group: 'shape',
    label: '📏 ثبّت مقاس السعر ورقم الباركود',
    hint:
      'دلوقتي مقاس السعر والرقم بيتغيّر حسب طول اسم الصنف — لحد 20% فرق ' +
      'بين ملصق وملصق. المفتاح ده بيخليهم ثابتين على كل الملصقات ' +
      '(السعر 2.6مم والرقم 2.4مم)، وبيضيّق المسافة بينهم عشان الاسم ' +
...
//
// ============================================================
// ⚠️⚠️ ليه الافتراضي 1.9 مش 2.7
// ============================================================
// أول نسخة خلّيته 2.7 (= اللي بيطبع دلوقتي) عشان "مايغيّرش حاجة".
// والنتيجة إن المفتاح بقى **مايعملش حاجة لما تفتحه** — وده افتراضي
// غلط: مفتاح لما يتفتح لازم يشتغل.
//
// و1.9 **مش اختيار ذوق، ده سقف المساحة**. القياس على أسماء المحل
// الحقيقية (من الصور اللي اتبعتت):
//
//   الرقم | كوكير(44ح) Sencse(36ح) ماليزي(18ح) اسدال(25ح) Hejap(17ح) | الفرق
//   مقفول |   1.94       1.94        2.70        1.94       2.68     |  28%
//    2.7  |   1.94       1.94        2.70        1.94       2.68     |  28%  ← مالوش أثر
//    2.4  |   1.94       1.94        2.40        1.94       2.40     |  19%
//    1.9  |   1.90       1.90        1.90        1.90       1.90     |   0%  ✅
//
// السبب: مساحة الاسم في الملصق **4.6مم** تقريبًا (الباقي للباركود
// والسعر). والاسم الطويل محتاج سطرين:
//     سطرين × 1.94 = 4.66مم  ← بيدخل بالعزق
//     سطرين × 2.70 = 6.48مم  ← مش موجودة أصلًا
// يعني الـ2.7 موجود **بس** في الأسماء اللي بتدخل سطر واحد. وأول ما
// الاسم يحتاج سطرين، السقف 1.94 مهما عملنا.
//
// فـ"موحّد وكبير" مع بعض **مستحيل** — مش قرار، ده ارتفاع الملصق.
const PRINT_NAME_MM_KEY = 'tazweed_print_name_mm';
const PRINT_NAME_MM_DEFAULT = 1.9;
...
async function maybeRasterizeSheet(type, list, sizeOptions) {
  ...
  // ⚠️⚠️ مفتاح "خام" **بيشغّل** الصورة لوحده
  // ------------------------------------------------------------
  // المسار الخام بيبعت **صورة** (ESC/POS raster)، فمن غير الرسم مالوش
  // أي معنى. أول نسخة كانت بتشترط مفتاح الصورة مفتوح كمان، والنتيجة إن
  // اللي يفتح "خام" لوحده مايحصلش عنده حاجة — والورقة تروح كنص، ولو
  // طلعت أتقل من حد الرسالة تتفتح نافذة طباعة المتصفح.
  //
  // ده نفس الفخ اللي حصل بالظبط في مفتاح مقاس الاسم، وبنشيله من الاتنين:
  // المفتاح اللي بيحتاج مفتاح تاني عشان يشتغل = مفتاح بايظ.
  if (typeof getPrintTweak !== 'function') return null;
  if (!getPrintTweak('sheetImage') && !getPrintTweak('sheetRaw')) return null;
...
// ============================================================
// 📏 مقاس الورق المتظبط في تعريف الويندوز
// ============================================================
// ليه ده موجود: عطل قص ورقة التزويد اتشخّص **أربع مرات غلط**، وكل مرة
// كان التشخيص مبني على تخمين حاجة على الجهاز إحنا مش شايفينها. أول مرة
// اتحلّت فيها نقطة بشكل قاطع كانت لما نشرنا **رقم نسخة QZ** وشُفناه —
// وطلع إن التلات أجهزة على نفس النسخة، فالتشخيص ده مات.
//
// اللي فاضل من التفسيرات: **فورم الورق في تعريف الويندوز مختلف من جهاز
// لجهاز**. والرقم ده هو اللي هيأكّده أو يقتله زي ما النسخة عملت بالظبط.
//
// ⚠️ الدالة **مش** بتخمّن اسم الحقل: لو ماعرفتش تلاقي المقاس، بترجّع
// أسماء الحقول اللي لقيتها. عشان لو الشكل اتغيّر في نسخة QZ، نشوف
// إيه الموجود فعلًا بدل ما نخمّن تخمينة خامسة.
async function readPrinterPaper(printerName) {
  if (!printerName) return '';
  try {
    if (typeof qz === 'undefined' || !qz.printers || typeof qz.printers.details !== 'function') return '';
    const all = await qz.printers.details();
    const list = Array.isArray(all) ? all : [all];
    const hit = list.find((d) => d && String(d.name || '') === String(printerName));
    if (!hit) return '';

    // الشكل المتوقع: كائن فيه عرض وطول بالملليمتر
    const sz = hit.size || hit.paper || hit.media || null;
    if (sz && (sz.width || sz.height)) {
      return `${sz.width || '?'}×${sz.height || '?'}مم`;
    }
    if (typeof hit.density !== 'undefined' && (hit.width || hit.height)) {
      return `${hit.width || '?'}×${hit.height || '?'}مم`;
    }
    // ماعرفناش — نرجّع اللي موجود عشان نبني عليه المرة الجاية
    return 'الحقول: ' + Object.keys(hit).join(',').slice(0, 60);
  } catch (err) {
    console.warn('تعذّرت قراءة مقاس ورق التعريف:', err);
    return '';
  }
}

// ============================================================
// 🧪 إرسال ورقة التزويد **خام** (ESC/POS)
// ============================================================
// الشرح الكامل ليه وإزاي عند مفتاح sheetRaw في PRINT_TWEAKS فوق.
//
// ⚠️⚠️ تلات نقط لازم تفضل زي ما هي:
//
//   ١) الإعداد **من غير مقاس خالص** — `qz.configs.create(name)` وبس.
//      أي مقاس هنا بيرجّعنا لموديل الصفحة بتاع الويندوز، وهو بالظبط
//      اللي بنهرب منه. (نفس شكل الإعداد اللي مسار TSPL بيستخدمه.)
//
//   ٢) بنبعت **الصورة PNG** وQZ بيحوّلها. لو حوّلناها إحنا كنا هنبعت
//      ~347 كيلو من النقط الخام — أكبر من حد الرسالة بسبع مرات.
//
//   ٣) **تقديم ورق وقص في الآخر**. لما بنتخطى التعريف، بنتخطى معاه
//      أمر القص اللي كان بيبعته آخر كل طبعة — فمحدش بقى بيقص.
//
// ⚠️⚠️ النقطة (٣) اتغيّرت بعد تجربة على ورق حقيقي (v0.77.9)
// ------------------------------------------------------------
// أول نسخة بعتت **تقديم ورق بس من غير قص**، والسبب المكتوب وقتها كان:
// "لو الطابعة مالهاش قصّاصة الأمر ممكن يطبع رموز غريبة".
//
// النتيجة على الورق: *"الورقة بتطبع الاخر وتمام بدون تصغير بس بتقف في
// الماكينه من غير م تتقض"*. يعني المسار الخام نجح، والاحتياط ده هو
// اللي وقف الشغل.
//
// والاحتياط نفسه كان في غير محله: المسار ده أصلًا **مقفول ورا مفتاح**
// وبيروح لطابعة ورقة التزويد بس، وهي XP-80C على التلات أجهزة — وكانت
// بتقص عادي أيام ما التعريف هو اللي بيبعت الأمر. يعني القصّاصة موجودة
// ومتأكدين منها، مش تخمين.
//
// `GS V 66 n` (الوظيفة B) = قدّم n نقطة وبعدين اقص قص جزئي. الفيرموير
// هو اللي بيحسب المسافة بين رأس الطباعة والسكينة، فمابنحسبهاش إحنا.
//
// بترجّع true لو اتبعتت، false لو محتاجين نرجع للطريقة العادية.
//
// ⚠️ ترتيب البايتات مهم: التقديم الأول عشان آخر سطر يخرج من تحت الرأس،
// وبعدين القص.
const SHEET_RAW_FEED = '\x1B\x64\x03'; // ESC d 3  = قدّم 3 سطور
const SHEET_RAW_CUT = '\x1D\x56\x42\x00'; // GS V 66 0 = قص جزئي

async function printSheetRaw(printerName, image) {
  if (!printerName || !image || typeof image !== 'string') return false;
  if (typeof qz === 'undefined' || !qz.configs || !qz.print) return false;
  try {
    const config = qz.configs.create(printerName);
    await qz.print(config, [
      {
      ...
      { type: 'raw', format: 'command', flavor: 'plain', data: SHEET_RAW_FEED + SHEET_RAW_CUT },
    ]);
    return true;
  } catch (err) {
    console.warn('تعذّر إرسال ورقة التزويد خام:', err);
    return false;
  }
}
...
async function tryPrintViaQZ(type, jobs, sizeOptions, onProgress) {
  ...
  try {
    ...
    // ============================================================
    // 🧪 ورقة التزويد **خام** — الشرح الكامل عند مفتاح sheetRaw فوق
    // ============================================================
    // ⚠️ فرع مقفول على ورقة التزويد بس، وبيخرج من الدالة بنفسه. الملصق
    // مابيعديش من هنا خالص — ولا سطر من مساره اتغيّر.
    if (rast && type === 'restock' && getPrintTweak('sheetRaw')) {
      const sent = await printSheetRaw(printerName, list[0] && list[0].image);
      if (sent) return true;
      // ⚠️ الرجوع للطريقة العادية **بيتقال** مش بيحصل في سكوت — ده
      // الفرق بين "المفتاح مانفعش" و"المفتاح شغّال وانت مش واخد بالك".
      showPrintNotice('📄 الإرسال الخام مانفعش — الورقة اتطبعت بالطريقة العادية.', 9000);
    }
    ...
    // ============================================================
    // ⭐ مقاس **برّه فورم التعريف** — QZ Tray 2.2.6+
    // ============================================================
    // بيشيل حد المساحة المطبوعة ويلغي التحقق من الصفحة، فاللي بنبعته
    // بيتطبع بمقاسه بالظبط بدل ما التعريف يتصرّف فيه.
    //
    // ⚠️ بيشتغل مع **الصور بس** (اتعمل في PrintImage.java لوحده)، وQZ
    // الأقدم بيتجاهله في صمت.
    //
    // ------------------------------------------------------------
    // ⚠️⚠️ ليه اتوسّع للملصقات كمان (v0.73.2)
    // ------------------------------------------------------------
    // ملصق مطبوع من المحل: العمود اليمين **مقصوص** — "Kuwaiti 12" بدل
    // "Kuwaiti 120"، و"1063210" بدل "10632103".
    //
    // والصورة اللي إحنا بنبعتها **سليمة تمامًا** (مقيسة): 304×200 نقطة
    // بالظبط، هامش 12 نقطة شمال و13 يمين، والخانات الأربعة كاملة.
    // يعني الصورة بتخرج كاملة **وبتتقص بعد كده**.
    //
    // وشكوى QZ #1413 بتوصف ده بالحرف: "التحجيم مفتوح → الصورة تطلع
    // صغيرة، التحجيم مقفول → **الصورة تتقص**؛ والسبب إن المساحة القابلة
    // للطباعة متظبطة أصغر من اللازم". وإحنا التحجيم عندنا **مقفول**
    // (مفتاح noScale) — فالحالة التانية بالظبط.
    //
    // و`custom` هو التعديل اللي اتعمل **ردًا على الشكوى دي بالذات**.
    //
    // ⚠️ والكثافة والسرعة على الطابعة اتجرّبوا خلاص ومانفعوش — فالسبب
    // مش في الماكينة.
    //
    // ⚠️ بيتحط لما **كل** الصفحات صور: الصورة بتتبعت عشان تتطبع نقطة
    // بنقطة، فأي تصرّف من التعريف فيها غلط بحكم التعريف. والـHTML
    ...
    if (tooBig) {
      ...
      if (type === 'restock') {
        // ⚠️⚠️ ده كان بيرجع **في صمت**، والمستخدم يلاقي نافذة طباعة
        // المتصفح فاتحة على الكمبيوتر ومش فاهم ليه — واتبلّغ بالنص:
        // "في امر بيروح ل جهاز الكمبيوتر بيتعمل معاينة على جهاز
        // الكمبيوتر نفس المشكلة اللي قبل كده".
        //
        // السبب الحقيقي: الورقة راحت **كنص** (مفتاح الصورة مقفول على
        // الجهاز ده)، والنص طلع أتقل من حد رسالة QZ — فمافيش طريقة
        // توصلها للطابعة مباشرة غير إنها تترسم كصورة.
        //
        // الرسالة بتقول **السبب والحل**، مش بس إن فيه مشكلة.
        const kb = Math.round(sizeOf(tooBig) / 1024);
        const cap = Math.round(QZ_MAX_MESSAGE_BYTES / 1024);
        showPrintNotice(
          `📄 ورقة التزويد طلعت ${kb} كيلو والحد ${cap} — فاتحصلش تتبعت للطابعة مباشرة، ` +
            'واتفتحت نافذة طباعة المتصفح بدالها. ' +
            'افتح مفتاح **"ورقة التزويد كصورة"** (ومعاه **"خام"**) في إعدادات الطابعة ' +
            'على الجهاز اللي بيطبع، وساعتها هتروح للطابعة على طول.',
          16000
        );
        return false;
      }
...
<div style="font-size:12px; font-weight:500; margin-bottom:4px;">🧪 عيّنة خطوط الطابعة</div>
        ...
        <div class="pset-sec">
          ...
          ${PRINT_TWEAK_GROUPS.map(
            ...
            ${PRINT_TWEAKS.filter((t) => t.group === g.key).map(
            (t) => `
            ...
            <div style="display:flex; gap:8px; align-items:center; padding:0 0 8px 26px;
              ...
              <div style="font-size:10px; color:var(--text-muted); line-height:1.6; width:100%;">
                الافتراضي <strong>1.9</strong> — وده <strong>أكبر رقم يخلي كل الأسماء متساوية</strong>.
                أي رقم أكبر، الأسماء اللي بتاخد سطرين مش هتقدر توصله (مساحة الاسم 4.6مم بس)
                فيرجع الاختلاف. تقدر تكبّره لو أسماءك كلها قصيرة.
              </div>
            </div>`
                : ''
            }${
  ...
  overlay.querySelectorAll('[data-tweak]').forEach((box) => {
    box.addEventListener('change', () => {
      const key = box.getAttribute('data-tweak');
      setPrintTweak(key, box.checked);
      // ============================================================
      // ⚠️⚠️ مفتاحين اتدمجوا في واحد: "خام" و"كصورة"
      // ============================================================
      // كانوا مفتاحين لنفس الهدف (الورقة الطويلة تطلع كاملة)، والخام
      // **مش بيشتغل** من غير الصورة أصلًا — فكان لازم تفتح الاتنين مع
      // بعض وتفتكر الترتيب. واتبلّغ بالنص: "جربت المفتاحين مع بعض".
      //
      // دلوقتي مفتاح واحد ظاهر (sheetRaw) وهو بيكتب الاتنين. وsheetImage
      // فضل موجود في الكود عشان الأجهزة اللي فاتحاه لوحده دلوقتي تفضل
      // شغّالة زي ما هي لحد ما المفتاح يتلمس.
      if (key === 'sheetRaw') setPrintTweak('sheetImage', box.checked);
    });
  });
