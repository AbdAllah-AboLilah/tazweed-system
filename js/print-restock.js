...
// ============================================================
// ⭐ تظليل خفيف — لورقة "اللي خلص بس"
// ============================================================
// المشكلة اللي بيحلها: في ورقة كل درجاتها خلصت، التظليل العادي بيتحط
// على **كل** خانة فبياكل المكان اللي بتكتب فيه الكمية.
//
// ⚠️ وشيله خالص **مش** الحل — اتطلب صراحةً:
//   "انا عاوز الورقة يبقي فيها تظليل حتي لو خفيف ... عشان انا لو
//    ماخدتش بالي من الورقة من فوق ممكن افتكرها متاحه."
// يعني التظليل شغله إنك تعرف بعينك من غير ما تقرا العنوان.
//
// ⚠️⚠️ و"أخف" هنا معناها **خطوط أقل**، مش لون أفتح. الطابعة الحرارية
// أبيض/أسود بس — أي رمادي بيطلع باهت ومنقّط (نفس درس مفتاح blackwhite).
// فسُمك الخط زي ما هو بالظبط (2.2)، والمسافة بينهم بس هي اللي اتوسّعت
// من 7 لـ16 → الحبر نزل من ~31% لـ~14%، والخطوط نفسها بتتطبع بنفس
// وضوح القديمة لأنها بنفس السُمك.
const HATCH_LITE_ID = 'hxl';

// بيتحط مرة واحدة في أول <body>. عرضه وطوله صفر فمش بياخد أي مكان.
// ⚠️ النمطين الاتنين بيتعرّفوا مع بعض (≈150 بايت زيادة للورقة كلها، مش
// للخانة) — أبسط وأأمن من إننا نقرر أنهي نمط نبعت حسب نوع الورقة.
const HATCH_DEFS = `<svg width="0" height="0" style="position:absolute" aria-hidden="true"><defs>` +
  ...
  `</pattern>` +
  `<pattern id="${HATCH_LITE_ID}" width="16" height="16" patternUnits="userSpaceOnUse" patternTransform="rotate(-45)">` +
  `<rect width="2.2" height="16" fill="#000"></rect>` +
  `</pattern></defs></svg>`;
...
const HATCH_CELL_LITE = `<svg class="hatch"><rect width="100%" height="100%" fill="url(#${HATCH_LITE_ID})"/></svg>`;
...
function buildRestockHTML(cat, grades, groupName, withBase, filterMode) {
  ...
  // ============================================================
  // ⚠️⚠️ ورقة "اللي خلص بس" بتظليل **خفيف**
  // ============================================================
  // في الورقة دي كل الخانات مظللة (كلهم خلصوا فعلًا)، والتظليل العادي
  // بياكل المكان اللي بتكتب فيه الكمية.
  //
  // ⚠️ بس شيله خالص غلط: التظليل هو اللي بيقولك "دي خلصت" بعينك من غير
  // ما تقرا العنوان — والورقة من غيره ممكن تتقري غلط على إنها متاحة.
  // فبيفضل موجود بخطوط أقل. الشرح الكامل عند HATCH_LITE_ID فوق.
  //
  // ⚠️ والورقة الكاملة وورقة "المتاح بس" **زي ما هما بالحرف**: هناك
  // المظلل قلة وسط السليم، فمحتاج يبقى صارخ عشان عينك تلقاه.
  const hatchCell = filterMode === 'out' ? HATCH_CELL_LITE : HATCH_CELL;
...
// ============================================================
// ⚠️⚠️ النافذة المخفية بتوقّف إطارات الرسم — والطباعة عن بُعد كلها بتحصل
//      والنافذة مخفية
// ============================================================
// العطل اللي بيتحل، اتبلّغ بالنص: "جهاز الكمبيوتر لما بطبع منه مبيعملش
// معاينه ولما ببعت من التليفون في ورق بيطلع ب معاينه وورق من غير
// معاينه"، ومعاه: "النظام انا مثبته علي سطح المكتب وفي نفس الوقت في
// برامج تانيه شغاله علي جهاز الكمبيوتر".
//
// والجملتين دول مع بعض هما الدليل: الطباعة من الكمبيوتر **وانت قدامه**
// شغّالة دايمًا، والطباعة من التليفون **والنافذة مخفية** هي اللي بتلخبط.
//
// القياس:
...
//
// السبب: الدالة دي كانت بتستنى `requestAnimationFrame`، وكروم بيوقّفه
// **خالص** للنافذة المخفية (مش بيبطّئه — بيوقّفه). فالوعد عمره ما
// بيتحقق، ورسم الورقة بيتعلّق، والطلب كله بيقف مستني.
//
// ⚠️ ودي حالة الاستعمال الأساسية مش حالة نادرة: الطباعة عن بُعد معناها
// بالتعريف إن محدش واقف قدام الجهاز، والنافذة ورا برامج تانية.
//
// الإصلاح: كل انتظار بيتسابق مع مؤقّت. الإطار بيكسب وهو موجود (فالوضع
// العادي مايتغيّرش ولا مللي ثانية)، والمؤقّت بيكسب لما الإطارات تقف —
// فالقياس بيكمّل بدل ما يتعلّق. ومعاه سقف بالثواني مش بالإطارات، لأن
// "٩٠ إطار" في نافذة مخفية ممكن تبقى دقيقة ونص أو مالا نهاية.
const SHEET_LAYOUT_MAX_MS = 2500;
...
async function settledSheetHeight(doc) {
  const nextFrame = () =>
    new Promise((r) => {
      ...
      // ⚠️ حزام الأمان. المؤقّت في النافذة المخفية بيتبطّأ لثانية تقريبًا،
      // بس **بيفضل شغّال** — بعكس الإطارات اللي بتقف تمامًا.
      setTimeout(fin, 32);
    });
  ...
  let last = -1;
  let stable = 0;
  // ⚠️ سقف بالثواني كمان: العدّ بالإطارات لوحده مالوش معنى لما الإطارات
  // نفسها تبقى بثانية لكل واحد.
  const deadline = Date.now() + SHEET_LAYOUT_MAX_MS;
  for (let i = 0; i < SHEET_LAYOUT_FRAMES && Date.now() < deadline; i++) {
    await nextFrame();
