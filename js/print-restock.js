...
async function renderSheetImage(html) {
  try {
    ...
    // ============================================================
    // ⚠️⚠️ `sandbox` من غير allow-scripts — ده إصلاح عطل مش تشديد أمان
    // ============================================================
    // العطل اللي بيتحل، اتبلّغ بالنص: "لو انا مش مخلي النظام في الخلفيه
    // الورقة بتطبع عادي انما لما يبقي في الخلفيه وافتح برنامج تاني لازم
    // بيجي امر معاينة ل جهاز الكمبيوتر".
    //
    // السبب: ورقة التزويد جوّاها سطر بيقول للمتصفح "اطبع لوحدك بعد 300
    // مللي" (موجود عشان الطباعة من نافذة المتصفح — شوف buildRestockHTML).
    // وإحنا بنكتب **نفس الورقة دي** في الإطار ده عشان نقيس طولها،
    // فالسطر بيشتغل جوّه الإطار ويفتح نافذة طباعة الويندوز.
    //
    // وليه الخلفية بالذات:
    //   • النافذة قدامك → القياس بيخلص في ~80 مللي، والإطار بيتشال
    //     **قبل** الـ300 مللي. فالأمر عمره ما بيشتغل.
    //   • النافذة ورا برنامج تاني → المتصفح بيبطّئ كل المؤقتات لثانية
    //     تقريبًا، فالقياس بياخد ثواني والإطار بيعيش لبعد الـ300 —
    //     والأمر بيشتغل.
    //
    // ⚠️ وشيل الـ<script> تحت **مش كفاية**: هو بيتشال بعد القياس، والسطر
    // بيكون اشتغل وسجّل نفسه على onload من ساعة doc.write. شيل العنصر
    // مابيلغيش التسجيل. المنع الوحيد المضمون إن السكربت **مايشتغلش من
    // الأصل**.
    //
    // ⚠️ و`allow-same-origin` لازم تفضل: من غيرها مانقدرش نقرا
    // contentDocument ولا نقيس حاجة. والورقة مافيهاش أي سكربت محتاجينه —
    // السطر ده هو الوحيد، وهو اللي بنمنعه.
