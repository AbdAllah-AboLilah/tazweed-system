      ...
      overviewCoalesceTimer = setTimeout(() => {
        ...
        allGradesCache = latest.docs.map((d) => {
          ...
          return {
            ...
            name: g.name,
            number: g.number,
            // ⚠️⚠️ المجموعة — اتبلّغ بالنص: "في حركة المخزن لما بتفتح فئة
            // فيها مجموعات بيكتب الدرجات بدون ما اشوف الدرجة دي تخص انهي
            // مجموعة في الفئة".
            //
            // والسبب كان هنا بالظبط: النسخة المختصرة دي كانت **بتشيل**
            // الحقل ده، فشاشة الحركة مكانتش عندها المجموعة أصلًا عشان
            // تكتبها. في فئة فيها "كيوي" و"نصار"، "درجة 56" بتطلع مرتين
            // ومحدش عارف أنهي فيهم اللي راكدة.
            //
            // ⚠️ التكلفة صفر: الحقل جاي في نفس المستند اللي اتقرا أصلًا.
            group: g.group || '',
...
function recomputeOverview() {
  ...
  allGradesCache.forEach((g) => {
    ...
    // ============================================================
    // ⚠️⚠️ نسخة مكررة من الحساب = عطل بيستنى يحصل
    // ============================================================
    // كان هنا حساب **مكتوب تاني** للحد الحرج، وفيه `Number(g.criticalQty)
    // || DEFAULT` — والصفر بيتبلع في `||` ويرجّع 3.
    //
    // النتيجة اللي المستخدم شافها: بيحط الحد **صفر** (يعني من غير تنبيه)،
    // والدايرة البرتقالية جنب اسم الفئة **تفضل ظاهرة** بعدد الدرجات
    // الأساسية — وهو مش عارف ليه. `gradeCriticalQty` في app.js كانت
    // بتتعامل مع الصفر صح من الأول، بس الشاشة دي مكانتش بتناديها.
    //
    // 📌 القاعدة: حساب واحد في مكان واحد. النسخة التانية بتفضل صح شوية
