...
(async () => {
  ...
  // ⚠️ من v0.88.0 مفتاح البرنامج المساعد مفتوح افتراضيًا، فهو
  // بيتجرّب **قبل** QZ. فالسيناريو ده بقى ليه سببين حقيقيين مع بعض:
  // المساعد مش شغّال، والطابعة مش متظبّطة. والتنبيه بيقول اللي حصل
  // فعلًا — فالفحص بيقبل أي واحد فيهم، المهم إنه **بيقول سبب**
  // مش بيسكت.
  check('⭐⭐ وبيقول السبب (المساعد مش شغّال أو الطابعة مش متظبطة)',
    notice.sheet.indexOf('مش متظبّطة') !== -1 ||
      notice.sheet.indexOf('المساعد مش شغّال') !== -1, notice.sheet);
