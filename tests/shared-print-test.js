...
(async () => {
  ...
  // ============================================================
  // ⚠️⚠️ الأرقام اتغيّرت في v0.88.0 — والسبب مكسب مش عطل
  // ============================================================
  // الافتراضي بقى **صورة** بدل نص، وصورة الملصق أصغر بكتير من
  // الـHTML بتاعه. القياس:
  //     نص   → 20 رسالة، 5 صفحات في الرسالة
  //     صورة →  5 رسايل، 20 صفحة في الرسالة، أكبر رسالة 16 كيلو
  // (والحد 48 كيلو، فإحنا أبعد عنه من الأول.)
  //
  // ⚠️ فالرقم الثابت (20) كان **بيوصف** المسار النصّي، مش شرط.
  // الشروط الحقيقية اللي الفحص ده اتكتب عشانها متحطوطة تحت بالنص:
  // الطبعة اتقسّمت فعلًا، وولا وظيفة عدّت حجم الدفعة المظبوط.
  check('⭐ اتقسّموا على أكتر من وظيفة', big.jobs > 1, big);
  check('⭐⭐ وولا وظيفة عدّت حجم الدفعة المظبوط على الجهاز',
    big.maxPages <= big.perJob, big);
  // ⚠️ الرقم 5 كان بيوصف المسار النصّي كمان (شوف الشرح فوق). الشرط
  // متحطوط فوق بحجم الدفعة المظبوط، وده اللي بيتغيّر مع الإعداد.
  check('⭐ والحد الأقصى للصفحات في الوظيفة معقول', big.maxPages <= 40, big);
