...
(async () => {
  ...
  //
  // ============================================================
  // ⚠️⚠️ الاستثناء الوحيد — واقراه قبل ما تزوّد عليه
  // ============================================================
  // القاعدة فوق مبنية على افتراض واحد: **ورا كل مفتاح عملية في
  // السحابة**. المفتاح اللي تحت هو الاستثناء الحقيقي الوحيد:
  //
  //   downloadHelper → بيخفّي/يظهّر **رابط تحميل ملف ثابت** على
  //   نفس الموقع. مافيش أي قراءة ولا كتابة في Firestore وراه، يعني
  //   مافيش حاجة القواعد تمنعها أصلًا. والملف نفسه **عام على
  //   الإنترنت** لأي حد معاه الرابط — فالمفتاح بيرتّب الشاشة، مش
  //   بيأمّن الملف، وده مكتوب صراحةً عشان محدش يفتكره حماية.
  //
  // ⚠️ أي مفتاح جديد **مايتحطش هنا** إلا لو ينطبق عليه نفس الكلام
  // بالحرف: صفر تعامل مع السحابة. لو فيه أي قراءة أو كتابة وراه،
  // مكانه في firestore.rules مش في القايمة دي.
  const UI_ONLY_KEYS = ['downloadHelper'];
  const notInRules = allKeys.filter((k) => !UI_ONLY_KEYS.includes(k) && !rulesSrc.includes(`'${k}'`));
  check('⭐⭐ كل مفتاح موجود في firestore.rules', notInRules.length === 0, notInRules);
  // ⚠️ والاستثناء نفسه لازم يفضل مستثنى **بسبب**: لو حد ضاف المفتاح
  // في القواعد بعدين، يبقى بقى ليه تعامل مع السحابة ولازم يخرج من
  // القايمة دي ويترجع للحارس.
  const wrongly = UI_ONLY_KEYS.filter((k) => rulesSrc.includes(`'${k}'`));
  check('⭐ والمفتاح المستثنى لسه فعلًا مالوش أثر في القواعد', wrongly.length === 0, wrongly);
