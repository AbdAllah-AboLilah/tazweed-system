...
(async () => {
  ...
  const sent = await p.evaluate(async () => {
    ...
    return {
      ...
      batch: typeof getPrintBatchSize === 'function' ? getPrintBatchSize() : 5,
    };
  });
  ...
  // ⚠️⚠️ العدد **مش ثابت** عن قصد، والرقم الثابت هنا غلّط مرتين:
  //   • أول مرة: كان مكتوب 60 بالظبط، وأول ما اتغيّر مقاس الخط الفحص وقع
  //   • تاني مرة: كان مقسوم على 5 (مبني على حجم الملصق القديم)، وأول ما
  //     الصورة بقت 1 بت (أصغر 5 مرات) بقت صفحات أكتر تدخل في الرسالة
  //     الواحدة — فالعدد قلّ والفحص وقع، **والتعديل كان سليم**.
  //
  // فبنقارن بالحد الحقيقي (حجم الدفعة المظبوط) مش برقم متخمّن. واللي
  // يهم فعلًا: كل الصفحات وصلت، وكل رسالة تحت الحد، والتقسيم بيحترم
  // حجم الدفعة.
  check('⭐ اتقسّموا على وظايف صغيرة كلها تحت الحد',
    sent.messages >= Math.ceil(sent.pages / sent.batch) && sent.maxBytes <= sent.limit, sent);
  ...
  // ⭐ v0.36.0: الملصق النصّي بقى الافتراضي — كان أنضف على الورق وقتها.
  // ⚠️⚠️ v0.88.0: **اتقلب تاني** — الصورة بقت الافتراضي، عشان البرنامج
  // المساعد بياخد صور بس واتطلب إنه يبقى المسار الافتراضي. والمسار
  // النصّي لسه موجود ورا المفتاح زي ما الصورة كانت بالظبط.
  check('⭐ الافتراضي بقى الصورة (البرنامج المساعد)',
    back.dfltImage !== null && back.dfltIsHTML === false, back);
