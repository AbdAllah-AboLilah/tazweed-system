...
(async () => {
  ...
  // ============================================================
  // ⚠️⚠️ الافتراضي **اتقلب** في v0.88.0 — والاتجاه اتعكس عن قصد
  // ============================================================
  // كان: النص هو الافتراضي (من v0.36، بعد تجربة على ورق ضد صورة
  // النسخة القديمة اللي كانت بتتكبّر من QZ وتتنغمش).
  // بقى: الصورة هي الافتراضي — اتطلب بالنص "عاوزك تخلي مفاتيح المساعد
  // هي الافتراضي ومفتوحه"، والبرنامج المساعد بياخد **صور** بس.
  //
  // ⚠️ والملصق اتطبع من المساعد على ورق حقيقي قبل التغيير ده، والحكم
  // كان: "الملصق بيطبع عادي وطلع كويس".
  check('⭐⭐ الافتراضي بقى صورة مش نص (عشان البرنامج المساعد)',
    route.dfltIsText === false, route);
  check('⭐ والمفتاح لسه بيرجّع الصورة لو حبيت', route.imgIsImage, route);
  check('⭐⭐ وشاشة الطباعة بتاخد نفس الطريق', route.cartIsText === false, route);
