...
(async () => {
  ...
  // ============================================================
  // ⭐⭐⭐ الفتح بياخد من المحفوظ على طول — مش بيسأل الشبكة الأول
  // ============================================================
  // اتبلّغ بالنص: "الوقت كان في فتح الصفحة نفسها".
  //
  // النظام بيحمّل 27 ملف من نفس الموقع مع كل فتحة. وقبل كده كان بيسأل
  // الشبكة عن **كل واحد فيهم** في كل مرة، حتى لو مافيش حاجة اتغيّرت.
  //
  // القياس (خادم بتأخير 400 مث للملف، متصفح حقيقي، وسيط 3 فتحات):
  //     قبل .... 2444 مث
  //     بعد ....  319 مث      ← أسرع 7.7 مرة
  //
  // ⚠️ والتحديث لسه بيوصل في **فتحة واحدة** — اتقاس بتحديث وهمي
  // على خادم تجربة: النسخة اتغيّرت من 1.16.0 لـ9.9.9 بعد فتحة واحدة.
  // السبب إن التحديث مش بييجي من هنا أصلًا: install بينزّل الملفات
  // كلها من السيرفر في مخزن باسم النسخة الجديدة.
  const fetchPart = sw.slice(sw.indexOf("addEventListener('fetch'"));
  check('⭐⭐⭐ الفتح بيرجّع المحفوظ قبل ما يسأل الشبكة',
    /caches\.match\(request\)[\s\S]{0,120}if \(cached\) return cached;/.test(fetchPart), null);
  // ⚠️ من غير waitUntil المتصفح بيقفل الـService Worker بعد ما يرد،
  // فالتحديث الخلفي مابيخلصش ومابيتحفظش — والنظام يفضل على القديم للأبد.
  check('⭐⭐⭐ والتحديث الخلفي متمسّك بـwaitUntil (مش بيتقفل في نصه)',
    /event\.waitUntil\(background\)/.test(fetchPart), null);
  // ⚠️ دول اللي بيخلّوا التحديث يوصل رغم إن الفتح من المحفوظ.
  check('⭐⭐ ونزول الملفات وقت التحديث لسه من السيرفر (cache: reload)',
    /cache: 'reload'/.test(sw), null);
  check('⭐⭐ واسم المخزن فيه رقم النسخة (النسخة الجديدة مخزن جديد)',
    /CACHE_NAME = 'tazweed-' \+ SW_VERSION/.test(sw), null);
