...
// ============================================================
// ⚠️⚠️ المفتاح لازم يكون **متحطّط فعلًا**
// ============================================================
// من غيره كل الكود ده موجود ومش شغّال، والنظام بيقول للمستخدم إن
// الإشعار بيوصل والنظام مفتوح بس. الفحص ده بيمسك لو حد فضّاه بالغلط
// أو نسي يحطه بعد ما نسخ المستودع من جديد.
//
// ⚠️ والشكل كمان: مفتاح Web Push بيبدأ بـB وطوله 87 حرف. الرقم ده
// بيمسك اللصق الناقص — واللي بيطلّع خطأ غامض من المتصفح مش رسالة
// واضحة مننا.
const valuesSrc = fs.readFileSync(path.join(root, 'firebase-config-values.js'), 'utf8');
const vapid = (valuesSrc.match(/FIREBASE_VAPID_KEY = "([^"]*)"/) || [])[1] || '';
check('⭐⭐⭐⭐ مفتاح الإشعارات متحطّط', vapid.length > 0);
check(`⭐⭐⭐ وشكله سليم (بيبدأ بـB وطوله 87، وطوله دلوقتي ${vapid.length})`,
  vapid.length === 87 && vapid[0] === 'B', vapid.length);
