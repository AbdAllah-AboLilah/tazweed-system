...
//
// ⚠️ الفحص ده كان فيه سطر أحمر فترة ("وبيتقرا من غير سعر") واتضح إنه
...
// ============================================================
// فك ترميز الكود من الملصق المقسوم أربعة
// ============================================================
// بناخد **خلية واحدة** (الربع اللي فوق على الشمال) — زي ما الماكينة
// بتشوفها بالظبط. لو فككنا الورقة كلها، الأربع أكواد بيلخبطوا القارئ.
//
// ⚠️⚠️ وبنفك من **ناحية الكود** في الخلية، مش من الخلية كلها. اقرا
// السبب قبل ما ترجّعها:
//
// الفحص كان بيفك من الخلية كلها (كود + كلام)، وكان فاشل في حالة
// "بدون السعر" **وبس** — وفضل أحمر فترة واحنا فاكرينه عطل في الملصق.
// وهو مكانش:
//
//   • قارنّا مربع الكود في "عادي" و"بدون سعر" نقطة بنقطة →
//     **صفر اختلاف**. الكود واحد بالحرف. كل الفرق في الكلام على اليمين.
//   • وغطّينا الكلام بأبيض في نفس الصورة الفاشلة → اتفكّت على طول،
//     بكل الهوامش.
//   • والدليل القاطع: نفس الصورة بتتفك بهامش ٤ نقط وبتفشل بهامش ٨.
//     لو المشكلة في الكود، زيادة الهامش الأبيض مكانتش تفرق.
//
// يعني القارئ (jsQR) كان بيتلخبط في **الحروف العربي** ويفتكرها مربعات
// تحديد (finder patterns)، والهامش بيزحلق شبكة التحليل بتاعته فتبوظ.
// ده عيب في القارئ اللي بنفحص بيه، مش في الملصق — والماكينة الحقيقية
// بتتصوّب على الكود مش بتحاول تقرا اللاصقة كلها.
//
// ⚠️ والخطر اللي الفحص القديم كان **بيوهمنا** إنه بيمسكه (الكلام يزحف
// على الكود) اتعمله فحص مستقل وصريح تحت — شوف qrClearance.
// ⚠️ مافيش نسبة محفورة هنا. حدود الكود **بتتقاس** من الصورة نفسها
// (columnsOf تحت)، فلو التصميم زحلق الكود أو كبّره الفحص بيمشي معاه بدل
// ما يفشل من غير سبب. القياس الفعلي على مقاسنا النهارده:
//   • الكود      → أعمدة ١٢ لـ٧٤
//   • فراغ أبيض  → ٧٥ لـ٧٩  (٥ أعمدة)
//   • الكلام     → ٨٠ لـ١٣٩
//   • خط القص    → العمود ١٥١
const QR_GAP = 4;   // أقل فراغ أبيض بنعتبره "فاصل بين الكود والكلام"

// بترجّع الخلية كصورة رمادية مسطّحة (0 أسود / 255 أبيض).
function cellOf(dataUrl) {
  ...
  for (let y = 0; y < ch; y++) {
    ...
    for (let x = 0; x < cw; x++) {
      ...
      row.push((img.data[s]*0.299 + img.data[s+1]*0.587 + img.data[s+2]*0.114) < 128 ? 0 : 255);
    }
    g.push(row);
  }
  return { g, cw, ch };
...
// عدد النقط السودا في كل عمود — الأساس اللي بنقيس بيه حدود الكود.
function columnsOf(g, cw, ch) {
  const cols = [];
  for (let x = 0; x < cw; x++) {
    let n = 0;
    for (let y = 0; y < ch; y++) if (g[y][x] === 0) n++;
    cols.push(n);
  }
  return cols;
}

// آخر عمود في الكود = آخر عمود أسود قبل أول فراغ أبيض عرضه QR_GAP أو أكتر.
// ⚠️ أعمدة الكود عندنا **كلها** فيها نقط سودا (١٢ لـ٧٤ من غير أي عمود
// فاضي)، فأي فراغ بالحجم ده معناه إن الكود خلص فعلًا.
function qrRightEdge(cols, cw) {
  const first = cols.findIndex((c) => c > 0);
  if (first < 0) return { first: -1, right: -1 };
  let right = first, i = first;
  while (i < cw) {
    if (cols[i] > 0) { right = i; i++; continue; }
    let j = i;
    while (j < cw && cols[j] === 0) j++;
    if (j - i >= QR_GAP) break;
    i = j;
  }
  return { first, right };
...
function decodeCell(dataUrl, quiet) {
  const { g, cw, ch } = cellOf(dataUrl);
  // بنقص عند نص الفراغ الأبيض اللي بعد الكود — مش عند نسبة ثابتة.
  const { right } = qrRightEdge(columnsOf(g, cw, ch), cw);
  const side = Math.min(cw, right + Math.floor(QR_GAP / 2) + 1);
  const W = side + quiet * 2, H = ch + quiet * 2;
  const out = new PNG({ width: W, height: H });
  out.data.fill(255);
  for (let y = 0; y < ch; y++) for (let x = 0; x < side; x++) {
    const d = (((y + quiet) * W + (x + quiet)) << 2);
    out.data[d] = out.data[d+1] = out.data[d+2] = g[y][x]; out.data[d+3] = 255;
  }
...
// ============================================================
// الحارس الحقيقي: الكلام مايزحفش على الكود
// ============================================================
// ده اللي الفحص القديم كان المفروض يمسكه. بنقيس بالأعمدة:
//   qrRight  → آخر عمود فيه نقطة سودا **قبل** أول فراغ عرضه ٦ نقط
//   textLeft → أول عمود فيه نقطة سودا بعد الفراغ ده
// وبعدين بنتأكد إن الاتنين على ناحيتين مختلفين من خط القص المنطقي.
//
// ⚠️ الفراغ ٦ نقط مش رقم عشوائي: أضيق فراغ جوّه الكود نفسه وحدة واحدة
// (نقطتين على مقاسنا)، فـ٦ أكبر من أي فراغ داخلي وأصغر من المسافة بين
// الكود والكلام (٢٣ نقطة) — فبيفصل الاتنين من غير ما يقطع الكود نصين.
function qrClearance(dataUrl) {
  const { g, cw, ch } = cellOf(dataUrl);
  const cols = columnsOf(g, cw, ch);
  const { first, right } = qrRightEdge(cols, cw);
  let textLeft = -1;
  for (let x = right + 1; x < cw; x++) if (cols[x] > 0) { textLeft = x; break; }
  // ⚠️ gap هنا **للتشخيص بس، مايتفحصش عليه**: qrRightEdge أصلًا مابتقفش
  // غير عند فراغ ≥ QR_GAP، فـ(textLeft - right) عمره ما هيقل عنه — أي
  // فحص عليه هيبقى فحص بيعدّي دايمًا. اللي بيمسك الكلام الزاحف فعلًا هو
  // مدى qrRight تحت، لأن الكلام الملزوق بيتحسب امتداد للكود فالرقم بيقفز.
  return { qrLeft: first, qrRight: right, textLeft, gap: textLeft - right, cw };
}
...
(async () => {
  ...
  // ⚠️⚠️ الحارس اللي كان ناقص: **الكلام مايزحفش على الكود**. الشرح
  // الكامل عند qrClearance فوق — الفحص القديم كان بيوهمنا إنه بيغطّي ده
  // وهو كان بيفشل لسبب تاني خالص (القارئ بيتلخبط في الحروف العربي).
  [['عادي', quarter.normal], ['من غير سعر', quarter.noPrice], ['اسم طويل', quarter.longName]]
    .forEach(([label, url]) => {
      const c = qrClearance(url);
      // ⚠️ القياسات دي **مقاسة** مش متخيّلة: الكود ١٢..٧٤ والكلام من ٨٠،
      // في خلية عرضها ١٥٢ نقطة. المدى الواسع عشان تعديل بسيط في التصميم
      // مايفشّلش الفحص من غير سبب، وضيّق كفاية إن العطلين الحقيقيين
      // يقعوا فيه:
      //   • الكلام يزحف على الكود → الاتنين بيبقوا كتلة واحدة، فآخر
      //     عمود بيقفز لـ١٣٩ (بره المدى)
      //   • الكود يختفي → أول عمود بيبقى ٨٠ بدل ١٢ (بره المدى)
      check(`⭐⭐ (${label}) الكود بادئ من الشمال`, c.qrLeft > 0 && c.qrLeft <= 20, c);
      check(`⭐⭐ (${label}) وخالص قبل نص الخلية`, c.qrRight >= 40 && c.qrRight <= 95, c);
      check(`⭐⭐ (${label}) والكلام بعده مش فوقه`, c.textLeft > c.qrRight, c);
    });
