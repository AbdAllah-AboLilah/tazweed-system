...
(async () => {
  ...
  const r = await p.evaluate(async () => {
    ...
    out.badValues = Object.keys(PRINT_TWEAK_ROUTE).filter(
      // ⚠️ 'needsHelper' اتضافت في v0.98.0: مفتاح بيغيّر **شكل**
      // الملصق بس بيقرا من البرنامج المساعد، فمالوش أثر من غيره.
      // وهي مش طريق — عشان كده الفحص اللي تحت (مفتاحين بس بيغيّروا
      // الطريق) لسه بيعدّ 'helper' لوحدها.
      (k) => ['helper', 'qz', 'both', 'needsHelper'].indexOf(PRINT_TWEAK_ROUTE[k]) === -1
    );
